...
def _close_env(env_id: int):
    return close_env(academia_env_server.env, env_id)


@app.get("/health")
def health():
    return {"status": "ok", "service": "academia"}


@app.post("/create")
def create(create_query: CreateQuery):
    env = wrap_call(academia_env_server.create, create_query.task_id)
...
def step(step_query: StepQuery):
    result = wrap_call(academia_env_server.step, step_query.env_id, step_query.action)
...
def reset(reset_query: ResetQuery):
    result = wrap_call(academia_env_server.reset, reset_query.env_id, reset_query.task_id)
    if isinstance(result, JSONResponse):
        return result
    obs = wrap_call(academia_env_server.observation, reset_query.env_id)
